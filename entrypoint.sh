...
if [ -z "${INPUT_TOKEN:-}" ]; then
  ...
  echo "Create a fine-grained PAT with 'Metadata: Read-only' AND 'Contents: Read and write'" >&2
  echo "on the target repo, store it as a repo secret (e.g. STAR_HISTORY_TOKEN)," >&2
  echo "and set 'token:' in your workflow." >&2
  exit 1
fi
